...
export default function TripsHistory() {
  ...
  if (isLoading) {
    ...
    const combinedData = data?.getall
.map((item, index) => ({
    programme: item,
          ...
          combinedData?.map((history, index) => (
            ...
            >
              <ListItemIcon className="text-black">
                <img src="/assets/qr-code.png" className='h-8 w-8 ' alt="qrcode" />
              ...
              <ListItemText
                primary={
                  ...
                  >
                    ...
                    }}  className="cursor-pointer"  >{  history?.programme?.full_name} </p>
                    <Link
                      href={`/square/programme/participant?programe_id=${history?.programme?.id}`}
                    >
                      <div
