...
export default function EditProgrammeModal(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_ENDPOINT}programme/${props?.programmeId}`,
        json,
        {
          headers: {
        ...
        <Box sx={style}>
          <div className=" flex justify-center items-center bg-white p-6 overflow-y-auto">
            ...
            >
              ...
              <div className="py-5 space-y-6">
                <div className="flex space-x-3">
                  {programme?.manager?.map((data) => {
                    return (
                      <Chip
                        key={data}
                        label={`${data.manager_first_name}  ${data.manager_last_name}`}
                        // onDelete={
                        //   data.label === "React"
                        //     ? undefined
                        //     : handleDelete(data)
