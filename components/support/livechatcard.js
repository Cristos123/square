...
export default function LiveChatCard(props) {
  ...
  return !!isOpen ? (
    <div className="max-h-screen  h-screen md:h-auto  text-black md:shadow-md  w-screen md:w-1/2 lg:w-1/3 bg-gray-100 fixed top-0 left-0 md:left-[70%] md:transform md:-translate-x-1/2 md:top-4 lg:left-2/4 lg:transform lg:translate-x-1/3 left z-[999999] md:rounded-2xl">
      ...
      <div className="p-4 mt-72 overflow-y-auto md:h-64  flex w-full flex-col md:pb-16">
        {view == "WA" && (
          <>
            ...
            <div className="mt-8 text-gray-400 text-xs text-center">
              ...
              <a
                className="font-bold"
                href={`https://app.audstack.com/auth/signup?ref=${props?.data?.home_owner?.reg_no}`}
                target="_blank"
              >
                Audstack
              </a>
        ...
        {view == "MA" && (
          <>
            ...
            <a
              href={`mailto:tobiadiks@gmail.com`}
              className="mt-12 text-blue-600 cursor-pointer bg-white py-4 mx-auto px-4 w-fit rounded-full shadow shadow-blue-200 animate-bounce "
            >
      ...
      <div className="fixed flex w-full pt-2 bottom-0 left-0 bg-gray-100 text-black">
        <div className="w-fit mx-auto flex">
          <div
            className={` px-4 pb-2 border-b-4 cursor-pointer ${
              view == "WA" && "border-b-green-500"
            }`}
            onClick={() => setView("WA")}
          >
            <WhatsAppIcon />
    ...
    <div
      className=" flex space-x-2 fixed z-[999999] bottom-8 right-8 w-fit  cursor-pointer"
      onClick={() => setIsOpen(true)}
    >
      <div className="p-2 text-white h-fit my-auto rounded-full  shadow-sm hidden md:flex bg-purple-700">
        <div className="h-2 w-2 mr-4 rounded-full bg-white my-auto"></div> What
        can we do for you?
      </div>{" "}
      <div className="rounded-lg h-12 w-12 bg-black flex ring-1 ring-gray-600">
        <ChatBubbleOutlinedIcon className="mx-auto h-10 w-10 text-white my-auto rounded-lg " />
      </div>
    </div>
  );
}
