...
export default function EditManagerModal(props) {
  ...
  }, [manager, managerId, props.open]);
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    delete manager.id;
    delete manager.created_at;
    delete manager.updated_at;
    ...
    try {
      ...
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_ENDPOINT}manager/${managerId}`,
        json,
        {
          headers: {
      ...
      if (response.status == 200 || response.status == 201) {
        toast("Dispatcher created successfully");
  ...
  return (
    <div>
      ...
      >
       ...
       type */}
          {/* INPUTS */}
          <div className="mb-4 w-fit  mx-auto">Edit Manager</div>
          <div className="mb-8 md:flex space-x-4 md:w-1/2 mx-auto">
            <div className="h-48 w-full bg-gray-200  mx-auto  mb-4">
              <FilePond
                ...
                allowMultiple={false}
                maxFiles={1}
                server={{ process: processs, revert }}
