...
export default function ProfileUploadModal(props) {
  ...
  console.log("props?.data?.idd", props?.data, { passengerPic });
  useEffect(() => {
  ...
  const handleResponse = (response) => {
    handleRichTextChange("portrait", response?.secure_url);
  };

  const resetResponse = () => {
    handleRichTextChange("portrait", "");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    delete passengerTripEdit.passenger;
    // delete status;

    const datas = {
      id: getPersengerId,
      portrait: passengerPic,
    };
    const json = JSON.stringify(passengerPic);
    try {
      toast("Please wait...");
      console.log(json, "props?.data?.id", props?.data?.tripId);

      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_ENDPOINT}users/profile-upload`,
        json,
        {
          headers: {
          ...
          <div className=" flex justify-center items-center bg-white p-6 overflow-y-auto">
            ...
            >
              ...
              <div className="mb-8 md:flex space-x-4 md:w-1/2 mx-auto">
                <div className="h-48 w-full bg-gray-200  mx-auto  my-4">
                  <FilePond
                    ...
                    onupdatefiles={setPortrait}
                    allowMultiple={false}
                    maxFiles={1}
                    server={{ process: processLuggageImage, revert }}
                    name="file" /* sets the file input name, it's filepond by default */
                    labelIdle='Upload portrait <span class="filepond--label-action">Image</span>'
                  />
                </div>
              </div>
