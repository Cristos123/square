...
import PaymentHistoryPassenger from "./Payments";
...
const HomePassenger = () => {
  ...
  const { data, error, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_ENDPOINT}users/${getPersengerId}`,
    fetcher
  );
  console.log({ data });
      ...
      <div className=" container px-4 flex flex-col">
        <div className="flex p-3 items-center">
          ...
          <div className="flex flex-col">
            <h1 className="md:text-3xl text-black px-2 font-bold ">
              {data?.full_name}
            </h1>
            <small className="text-xs px-2 text-black italic ">Passenger</small>
          </div>
        </div>
