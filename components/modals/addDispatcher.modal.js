...
export default function AddDispatcherModal(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.instadrop.com.ng/api/store-dispatcher`,
        json,
        {
          headers: {
    ...
    if (e.target.name == 'phone_1' || e.target.name == 'phone_2') {
      if (e.target.value?.trim().startsWith('0')) {
          setDispatcher((prev) => ({
              ...
              dispatcher_owner: props.storeId,
          }));
      } else {
          setDispatcher((prev) => ({
              ...prev,
  ...
  return (
    <div>
      ...
      >
       ...
       type */}
          {/* INPUTS */}
          <div className="mb-4 w-fit  mx-auto">Add a new dispatcher</div>
